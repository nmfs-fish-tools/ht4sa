    ...
    for(i in begin[mpi.comm.rank()]:end[mpi.comm.rank()])
    {
        ...
        # make same for both 17 & 18
            ...
            # update selex params
                ...
                idx_f16_tail = tail(grep("F16",rownames(tmp_ctl$size_selex_parms)),n=1)
                # if sex selex offset
                if(selex_options_df$male_option[i]==2)
                {
                    new_f17 = matrix(0,nrow=(3+2*selex_options_df$nodes[i])+4,ncol=14)
                } else {
                    new_f17 = matrix(0,nrow=3+2*selex_options_df$nodes[i],ncol=14)
                }
                ...
                tmp_nodes_loc = floor(seq(from=50,to=250,length.out=tmp_nodes)/5)*5
                ...
                tmp_ctl$size_selex_parms = rbind(tmp_ctl$size_selex_parms[1:idx_f16_tail,],new_f17,new_f17,tmp_ctl$size_selex_parms[(idx_f16_tail+1):nrow(tmp_ctl$size_selex_parms),])


            SS_writectl(tmp_ctl,paste0(dir_run,"control.ss"), version = "3.30", overwrite = TRUE)
            rm(list="tmp_ctl","idx_f17","idx_f18","idx_f16_tail","tmp_nodes","tmp_nodes_loc")
        ...
        # run SS
            ...
            if(TESTING)
            {
                ...
                if(length(grep("!!  Run has completed  !!",tmp,fixed=TRUE))==0){rm(list=c("tmp"));stop("Error: Bad selex settings.")}
            } else{
        ...
        if(!TESTING)
        {
            ...
            # extract metrics: LF fit
                selex_results_df$selex_par[i] = sum(new_f17$PHASE>0)+1
            ...
            # compress SS output
                ...
                system(paste0("powershell tar -czf ss_files.tar.gz ",paste0(tmp_files,collapse=" ")))

            #_____________________________________________________________________________________________________________________________
            # clean-up
                file.remove(tmp_files)
                rm(list=c("tmp_out","pointer","new_f17","r4ss_model_run","tmp_files","model_name"))
        }
    }
...
if (id == 0) {
  ...
  x <- mpi.remote.exec(run_ss_child,
                       begin,
                       end,
                       proj_dir,
                       dir_utility,
